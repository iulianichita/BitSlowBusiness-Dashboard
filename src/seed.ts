 ...
 */
async function seedClients(db: Database, count: number): Promise<number[]> {
	...
	for (let i = 0; i < count; i++) {
		...
		const password = await Bun.password.hash(name);
		const phone = faker.phone.number();
		const address = faker.location.streetAddress({ useFullAddress: true });

		const info = insertClient.run(name, email, password, phone, address);
...
function seedTransactions(
	...
	db.transaction(() => {
		for (let i = 0; i < count; i++) {
			...
			// Get current owner (seller) or null if it isn't known yet
			const sellerId = coinOwners[coinId] || null;

			if (sellerId === null) {
				// Get the owner (= client_id) from table coins
				const ownerId = db
					.prepare<number, number>(`
					SELECT client_id FROM coins
					WHERE coin_id = ?	
					`)
					.get(coinId);
				coinOwners[coinId] = ownerId;
			}
			...
			// Get BitSlow value
			// const coinValue =
			// 	db.query("SELECT value FROM coins WHERE coin_id = ?").get(coinId)?.value || 0;
			const coinValue =
				db
					.query<{ value: number }, [number]>(
						"SELECT value FROM coins WHERE coin_id = ?",
					)
					.get(coinId)?.value || 0;
